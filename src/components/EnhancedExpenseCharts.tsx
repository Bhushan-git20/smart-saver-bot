...
export const EnhancedExpenseCharts: React.FC<EnhancedExpenseChartsProps> = ({ transactions }) => {
  ...
  const exportToPDF = () => {
    ...
    pdf.text(`Total Income: ₹${summaryStats.totalIncome.toFixed(2)}`, 30, 95);
    pdf.text(`Total Expenses: ₹${summaryStats.totalExpenses.toFixed(2)}`, 30, 105);
    pdf.text(`Net: ₹${summaryStats.net.toFixed(2)}`, 30, 115);
  ...
  const exportToCSV = () => {
    ...
    XLSX.writeFile(wb, `transactions-${format(new Date(), 'yyyy-MM-dd')}.csv`, { bookType: 'csv' });
